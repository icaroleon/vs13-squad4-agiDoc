...
BEGIN
   EXECUTE IMMEDIATE 'DROP TABLE VS_13_EQUIPE_4.DEPARTAMENTS';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -942 THEN
         RAISE;
...
BEGIN
   EXECUTE IMMEDIATE 'DROP SEQUENCE VS_13_EQUIPE_4.SEQ_DEPARTAMENTS';
EXCEPTION
   WHEN OTHERS THEN
      IF SQLCODE != -2289 THEN
         RAISE;
...
CREATE TABLE VS_13_EQUIPE_4.DEPARTAMENTS (
	...
	ID_COMPETITOR NUMBER(38, 0),
  	...
  	CONSTRAINT CHK_DEPARTAMENTS_ATRIBUTION CHECK ((CASE WHEN ID_INSTITUTION IS NOT NULL THEN 1 ELSE 0 END + CASE WHEN ID_COMPETITOR IS NOT NULL THEN 1 ELSE 0 END) = 1),
  	CONSTRAINT PK_DEPARTAMENTS PRIMARY KEY (ID_DEPARTAMENT),
  	CONSTRAINT FK_DEPARTAMENTS_INSTITUTIONS FOREIGN KEY (ID_INSTITUTION) REFERENCES VS_13_EQUIPE_4.INSTITUTIONS(ID_INSTITUTION),
  	CONSTRAINT FK_DEPARTAMENTS_COMPETITORS FOREIGN KEY (ID_COMPETITOR) REFERENCES VS_13_EQUIPE_4.COMPETITORS(ID_COMPETITOR)
);

CREATE SEQUENCE VS_13_EQUIPE_4.SEQ_DEPARTAMENTS
START WITH 1
INCREMENT BY 1
NOCACHE NOCYCLE;
...
CREATE TABLE VS_13_EQUIPE_4.USERS (
  	...
  	ID_DEPARTAMENT NUMBER(38, 8) NOT NULL,
  	CONSTRAINT PK_USERS PRIMARY KEY (ID_USER),
  	CONSTRAINT FK_USERS_DEPARTAMENTS FOREIGN KEY (ID_DEPARTAMENT) REFERENCES VS_13_EQUIPE_4.DEPARTAMENTS(ID_DEPARTAMENT)
);
